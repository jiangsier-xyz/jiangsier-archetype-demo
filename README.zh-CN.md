...
### 构建数据访问层
jiangsier-archetype-demo 默认的数据库表设计只是半成品，仅包含基本的用户体系相关信息。请定制你的数据库，将表结构信息更新到 [schema.sql](https://github.com/jiangsier-xyz/jiangsier-archetype-demo/blob/main/${artifaceId}-dal/src/main/resources/sql/schema.sql)，修改 [generatorConfig.xml](https://github.com/jiangsier-xyz/jiangsier-archetype-demo/blob/main/${artifaceId}-dal/src/main/resources/mybatis-generator/generatorConfig.xml)，再使用 [mgb.sh](https://github.com/jiangsier-xyz/jiangsier-archetype-demo/blob/main/bin/mgb.sh) 来生成你的数据访问层。
...
如果想要进行本地调试，一般不会运行 helm 渲染，并且，许多服务的连接地址通常也不是 k8s 中自动部署的服务地址。你需要自行解决依赖服务（如 MySQL、Redis）的问题，并根据实际情况，手工维护一份 [application-private.yml](https://github.com/jiangsier-xyz/jiangsier-archetype-demo/blob/main/${artifaceId}-start/src/main/resources/application-private-sample.yml)，再在 IDE 的调试选项中加载它，就可以正常调试你的应用了。
> [mysql-local.sh](https://github.com/jiangsier-xyz/jiangsier-archetype-demo/blob/main/bin/mysql-local.sh) 可以帮助你运行/停止一个本地 MySQL，希望能有助于你的调试。
